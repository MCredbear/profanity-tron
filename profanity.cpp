...
int main(int argc, char **argv)
{
	try
	{
		...
		if (prefixCount < 0)
		{
			std::cout << "error: the number of prefix matches cannot be lower than 0 :<" << std::endl;
			return 1;
		}
		...
		if (suffixCount < 0)
		{
			std::cout << "error: the number of suffix matches cannot be lower than 0 :<" << std::endl;
			return 1;
		}
		...
		if(prefixCount == 0 && suffixCount == 0) {
			std::cout << "error: the number of prefix & suffix cannot be equal to 0 at the same time :<" << std::endl;
			return 1;
		}
		...
		if (vDevices.empty())
		{
			std::cout << "error: the GPU device is not detected, please make sure the cuda driver is installed :<" << std::endl;
			return 1;
		}
		...
		{
			std::cout << "  Program caching ..." << std::flush;
